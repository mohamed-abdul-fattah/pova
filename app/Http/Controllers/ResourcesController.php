...
class ResourcesController extends Controller
{
	...
	public function store(Request $request)
	{
        ...
        if (method_exists($resource, 'phones')) {
            foreach ($request->phones as $phone) {
                $resource->phones()->create(['phone' => $phone]);
            }
        }
	...
	{
        $resources = Resource::paginate(30);

	    return view('backend.resources.batchedit', compact('resources'));
	}
	...
	public function batchupdate(Request $request)
	{
		...
		{
            $resourcei = Resource::findOrFail($key);
            $resourcei->update($resource);
	    }
