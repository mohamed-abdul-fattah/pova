...
@foreach ($categories as $category)
       <div class="panel-body row">
            <div class='bio-row{{ $errors->has('categories['.$category->id.'][parent_id]') ? ' has-error' : '' }}'>
	{!! Form::label('categories['.$category->id.'][parent_id]', trans('category.ParentId')) !!}
	{!! Form::text('categories['.$category->id.'][parent_id]', $category->categories['.$category->id.'][parent_id], array('class' => 'form-control', 'required' => 'required')) !!}

	@if ($errors->has('categories['.$category->id.'][parent_id]'))
		<span class='help-block'>
			<strong>{{ $errors->first('categories['.$category->id.'][parent_id]') }}</strong>
		</span>
	@endif
</div>
